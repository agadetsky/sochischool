...
torch::Tensor get_maximum_spanning_arborescence(torch::Tensor weights,
                                                torch::Tensor lengths) {
  int n = weights.size(-1);
  int batch_size = weights.size(0);
  torch::Tensor adjs = torch::zeros({batch_size, n, n});
  for (int sample_idx = 0; sample_idx < batch_size; sample_idx++){
    int length = lengths[sample_idx].item<int>();
    vector<bool> disabled(length, false);
    vector<vector<int> > candidate_heads(length);
    vector<vector<double> > candidate_scores(length);
    for (int dst = 0; dst < length; dst++) {
      for (int src = 0; src < length; src++) {
        double score = weights[sample_idx][dst][src].item<double>();
        candidate_heads[dst].push_back(src);
        candidate_scores[dst].push_back(score);
      }
    }
    torch::Tensor heads = torch::zeros({length});
    double value = 0;
    ...
    heads = RunChuLiuEdmondsIteration(&disabled,
                                      ...
                                      &candidate_scores,
                                      heads, &value);
    // skip the first node, which we know is the root.
    for (int idx = 1; idx < length; idx++){
      int src = heads[idx].item<int>();
      int dst = idx;
      adjs[sample_idx][src][dst] = 1.0;
    }
  }
  return adjs;
}
